...
humps <- new %>%
  dplyr::filter(vernacularName == "Humpback Whale")

blues <- new %>% 
  dplyr::filter(vernacularName == "Blue Whale")

grays <- new %>% 
  dplyr::filter(vernacularName == "Gray Whale")
...
hump_shp <- whale_shp %>% 
    filter(vrnclrN == "Humpback Whale")

blue_shp <- whale_shp %>% 
    filter(vrnclrN == "Blue Whale")

gray_shp <- whale_shp %>% 
    filter(vrnclrN == "Gray Whale")
...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            helpText("Visualize observations of three endangered whale species recorded along the California Coast by volunteer naturalists from 2013-2018."),
    ...
    output$map <- renderLeaflet({
      ...
      gray_obs <- gray_shp %>%
        ...
        leaflet(width = "100%", height = "800px") %>%
            ...
            addMarkers(data = hump_obs, lng = humps$lon, lat = humps$lat,
                       popup = paste("Date: " = "date"),
                       clusterOptions = markerClusterOptions(),
                       group = "Humpback Whales") %>% 
          addMarkers(data = blue_obs, lng = blues$lon, lat = blues$lat,
                     popup = paste("Date: " = "date"),
                     clusterOptions = markerClusterOptions(),
                     group = "Blue Whales")
    ...
    #                               "Total Sighted: " = "Whales Sighted", 
     #                              "Occurrence ID:   " = "OccrnID"),
      #              popup.format=list(OccrnID=list(format="s")))        
        ...
        #          overlayGroups = c("Humpback Whales", "Blue Whales")
